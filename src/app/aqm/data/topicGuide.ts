...
export const guideSections: GuideSection[] = [
  ...
  },
  {
    ...
    bullets: [
      ...
      'Coverage — probability of the antecedent across the dataset',
      'Calculate metrics from a small table',
      'Rule length distribution — high end lacks support, low end lacks confidence',
      'Interpret results in business context',
    ],
    questionIds: [3, 84, 85, 86, 87, 88, 89, 90, 99, 100, 101, 102, 103, 104, 105],
    studyTip: 'SUPPORT(A) = count(A) / total. CONFIDENCE(A→C) = count(both) / count(A). LIFT(A→C) = Confidence(A→C) / Support(C). COVERAGE = Support of just the antecedent. Lift = 1 → independent. Long rules lack support, short rules lack confidence — sweet spot is the middle.',
  },
  {
    id: 'ggplot',
